    ...
    def incremental_step(self, inputs):
        ...
        if kw > 1:
            if self.convolution_queue is None:
                self.convolution_queue = tf.zeros(
                    (batch_size, (kw - 1) + (kw - 1) * (dilation - 1), tf.shape(inputs)[2]))
            else:
                # shift queue
                self.convolution_queue = self.convolution_queue[:, 1:, :]

            # append next input
            self.convolution_queue = tf.concat([self.convolution_queue, tf.expand_dims(inputs[:, -1, :], axis=1)],
                                               axis=1)
...
class ResidualConv1dGLU():
    ...
    def __init__(self, residual_channels, gate_channels, kernel_size,
        ...
        self.conv1x1_out = Conv1d1x1(gate_out_channels, residual_channels, use_bias=use_bias,
                                     name='residual_block_out_conv')
        self.conv1x1_skip = Conv1d1x1(gate_out_channels, skip_out_channels, use_bias=use_bias,
                                      name='residual_block_skip_conv')
